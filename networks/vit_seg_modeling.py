    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        if self.hybrid:
            self.hybrid_model = ResNetV2(block_units=config.resnet.num_layers,
                                         width_factor=config.resnet.width_factor, in_channel=in_channels)
...
class Block(nn.Module):
    ...
    def load_from(self, weights, n_block):
        ...
        with torch.no_grad():
            query_weight = np2th(weights[pjoin(ROOT, ATTENTION_Q, "kernel").replace(
                '\\', '/')]).view(self.hidden_size, self.hidden_size).t()
            key_weight = np2th(weights[pjoin(ROOT, ATTENTION_K, "kernel").replace('\\', '/')]
                               ).view(self.hidden_size, self.hidden_size).t()
            value_weight = np2th(weights[pjoin(ROOT, ATTENTION_V, "kernel").replace(
                '\\', '/')]).view(self.hidden_size, self.hidden_size).t()
            out_weight = np2th(weights[pjoin(ROOT, ATTENTION_OUT, "kernel").replace('\\', '/')]
                               ).view(self.hidden_size, self.hidden_size).t()
    ...
    def forward(self, x):
        ...
        if mmg is not None:
            for i in range(len(features)):
                ...
                mmg_t = F.interpolate(mmg, size=(h, w), mode='bilinear', align_corners=False)
                features[i] = features[i]*mmg_t
...
CONFIGS = {
    ...
    'SwinViT': configs.get_swinViT_config(),
    # 'TransCEUS': configs.get_transceus_r50_b16_config(),
}

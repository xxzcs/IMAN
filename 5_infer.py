...
def gen_mmg(modality='US', root_path='../datasets/us_small', resume_path='', round_num=0, model_name='', model=None, suffix=''):
    if modality == 'US':
        ...
        if not os.path.exists(os.path.join(root_path, 'us_mmg_%s_th%s_kn%d_rd%d%s' %
                                           (model_name, str(thresh), kernel_size, round_num, suffix))):
            os.mkdir(os.path.join(root_path, 'us_mmg_%s_th%s_kn%d_rd%d%s' %
                                  (model_name, str(thresh), kernel_size, round_num, suffix)))
        if model is None:
            print('* USISNG LOADING MODEL TO GEN ON', os.path.join(root_path, 'us_mmg_%s_th%s_kn%d_rd%d%s' %
                                                                   (model_name, str(thresh), kernel_size, round_num, suffix)))
            if not os.path.exists(os.path.join(root_path, '%s_mmg_%s_th%s_kn%d_rd%d' % (modality.lower(), model_name, str(thresh), kernel_size, round_num))):
                os.mkdir(os.path.join(root_path, '%s_mmg_%s_th%s_kn%d_rd%d%s' %
                                      (modality.lower(), model_name, str(thresh), kernel_size, round_num, suffix)))
            print('* MMG LOADING', resume_path)
            model = torch.load(resume_path)
        else:
            print('* USING MODEL TO GEN ON', os.path.join(root_path, 'us_mmg_%s_th%s_kn%d_rd%d%s' %
                                                          (model_name, str(thresh), kernel_size, round_num, suffix)))
            model = model
        model.cuda()
        model.eval()
        ...
        for file in tqdm(os.listdir(base_path)):
            ...
            trans = breastdatasets2.get_us_val_aug(args.input_size)
            ...
            cv2.imwrite(os.path.join(root_path, 'us_mmg_%s_th%s_kn%d_rd%d%s' %
                        (model_name, str(thresh), kernel_size, round_num, suffix), file), margin)
        print('* FINISH MMG AT ROUND %d' % round_num)
    elif modality == 'CEUS':
        ...
        if not os.path.exists(os.path.join(root_path, 'ceus_mmg_%s_th%s_kn%d_fn%d_rd%d%s' %
                                           (model_name, str(thresh), kernel_size, frame_num, round_num, suffix))):
            os.mkdir(os.path.join(root_path, 'ceus_mmg_%s_th%s_kn%d_fn%d_rd%d%s' %
                                  (model_name, str(thresh), kernel_size, frame_num, round_num, suffix)))
        if model is None:
            print('* USISNG LOADING MODEL TO GEN CEUS MMG ON', os.path.join(root_path, 'ceus_mmg_%s_th%s_kn%d_fn%d_rd%d%s' %
                                                                            (model_name, str(thresh), kernel_size, frame_num, round_num, suffix)))
            if not os.path.exists(os.path.join(root_path, '%s_mmg_%s_th%s_kn%d_fn%d_rd%d%s' % (modality.lower(), model_name, str(thresh), kernel_size, frame_num, round_num, suffix))):
                os.mkdir(os.path.join(root_path, '%s_mmg_%s_th%s_kn%d_fn%d_rd%d%s' %
                                      (modality.lower(), model_name, str(thresh), kernel_size, frame_num, round_num, suffix)))
            print('* CEUS MMG LOADING', resume_path)
            model = torch.load(resume_path)
        else:
            print('* USING MODEL TO GEN ON', os.path.join(root_path, 'ceus_mmg_%s_th%s_kn%d_fn%d_rd%d%s' %
                                                          (model_name, str(thresh), kernel_size, frame_num, round_num, suffix)))
            model = model
        model.cuda()
        model.eval()
        ...
        for file in tqdm(os.listdir(base_path)):
            ...
            # trans = breastdatasets2.get_ceus_val_aug(args.input_size)

            # preprocessing_fn = smp.encoders.get_preprocessing_fn(
            #     'resnext50_32x4d', 'imagenet') if 'resnext50_32x4d' in smp.encoders.get_encoder_names() else None
            # process = get_preprocessing_CEUS(preprocessing_fn)
            image = np.concatenate(all_img, axis=2)
            img_c = image.copy()
            # args.activation = 'softmax'
            h, w, _ = img.shape
            args.in_channels = 5 #13
            trans = breastdatasets2.get_ceus_val_aug(args)
            # print(trans, img)
            ...
            margin[np.isnan(margin)] = 255
            margin = margin.astype(np.uint8)
            cv2.imwrite(os.path.join(root_path, 'ceus_mmg_%s_th%s_kn%d_fn%d_rd%d%s' %
                        (model_name, str(thresh), kernel_size, frame_num, round_num, suffix), file+'.png'), margin)
        print('* FINISH MMG AT ROUND %d' % round_num)


def one_round(round_num, ep_offset, model, metrics):
    ...
    if args.modality == 'CEUS':
        preprocessing_fn = None
        dataset = breastdatasets2.ceus_dataset(image_file=os.path.join(args.data_path, 'train', 'img'),
                                              gt_path=os.path.join(args.data_path, 'train', 'labelcol'),
                                              frame_num=args.in_channels,
                                              augmentation=breastdatasets2.get_ceus_train_aug(args),
        ...
        test_dataset = breastdatasets2.ceus_dataset(image_file=os.path.join(args.data_path, 'test', 'img'),
                                                   gt_path=os.path.join(args.data_path, 'test', 'labelcol'),
                                                   frame_num=args.in_channels,
                                                   augmentation=breastdatasets2.get_ceus_val_aug(args),
                                                   preprocessing=get_preprocessing_CEUS(preprocessing_fn),
                                                   use_mmg=args.model_name if args.use_mmg else None,
                                                   filter=args.filter,
                                                   kernel_size=args.kernel_size,
        ...
        dataset = breastdatasets2.us_dataset(image_file=os.path.join(args.data_path, 'train', 'img'),
                                            gt_path=os.path.join(args.data_path, 'train', 'labelcol'),
                                            augmentation=breastdatasets2.get_us_train_aug(args.input_size),
        ...
        test_dataset = breastdatasets2.us_dataset(image_file=os.path.join(args.data_path, 'test', 'img'),
                                                 gt_path=os.path.join(args.data_path, 'test', 'labelcol'),
                                                 augmentation=breastdatasets2.get_us_val_aug(args.input_size),
                                                 preprocessing=get_preprocessing_US(preprocessing_fn),
                                                 use_mmg=args.model_name if args.use_mmg else None,
                                                 filter=args.filter,
                                                 kernel_size=args.kernel_size,
    ...
    for epoch in tqdm(range(args.start_epoch, args.epochs)):
        ...
        if min_dice > valid_logs[criterion.__name__]:
            ...
            if args.save_checkpoint <= epoch:
                checkpoint_path = output_dir / ('checkpoint_min_dice_ep%d_rd%d.pth' % (epoch+ep_offset, round_num))
        ...
        writer.add_images("input_images", images, (epoch+ep_offset))
        writer.add_images("mask_ground_truth", masks, (epoch+ep_offset))
        writer.add_images("mask_prediction", masks_pred.int(), (epoch+ep_offset))
        writer.add_scalar('best/restart', round_num,  (epoch+ep_offset))
        ...
        for k, v in train_logs.items():
            writer.add_scalar('train/%s' % k, v, (epoch+ep_offset))
        writer.add_scalar('train/lr', optimizer.param_groups[0]["lr"], (epoch+ep_offset))
        for k, v in valid_logs.items():
            writer.add_scalar('val/%s' % k, v, (epoch+ep_offset))
        for k, v in test_logs.items():
            writer.add_scalar('test/%s' % k, v, (epoch+ep_offset))

        writer.add_scalar('best/dice_loss', min_dice, (epoch+ep_offset))
        # ##################################################################
...
def eval():
    ...
    for pth_name in pth_names:
        ...
        epochs = int(pth_name.split('_')[-2].replace('ep', ''))
        
        '''
        # only the last min_dice pth in each round are tesetd 
        new_pth_names = []
        for i in range(args.total_rounds):
            # ep_names = []
            dd = []
            ep_names = [name for name in pth_names if '_rd%d.pth' % i in name]
            for name in ep_names:
                dd.append(int(name.split('_')[-2].replace('ep','')))
            # print(max(dd))
            for name in ep_names:
                if 'ep%d' % max(dd) in name:
                    # print(name)
                    new_pth_names.append(name)

        for pth_name in new_pth_names:
            print('* Dealing ', pth_name)
            round_num = int(pth_name.split('_')[-1].replace('.pth', '').replace('rd', ''))
            epochs = int(pth_name.split('_')[-2].replace('ep', ''))
        '''
        ...
        # image save
        outpath = './%s_%s_%s_th%f' % (pth_name, args.task, args.model_name, args.thresh)
        ...
        for i, (image, mask_gt, real_ind) in enumerate(test_dataloader):
            ...
            # print(mask_gt_img.shape,mask_gt_img.dtype)
        ...
        print('%f\t%f\t%f\t%s\t%d\t%d\t%s\t%d' % ((sum(dice_scores)/len(dice_scores)), (sum(hd1_scores)/len(hd1_scores)),
                                                  #   (sum(hd2_scores)/len(hd2_scores)),
                                                  0,
                                                  args.resume.split('/')[-1], epochs, args.kernel_size, args.filter, len(dice_scores))
              )
        # break
...
def main(args):
    ...
    for r in range(args.total_rounds):
        ...
        if r == 0:
            ...
            gen_mmg(modality=args.modality, root_path=args.data_path,
                    resume_path=resume_path, round_num=r, model_name=args.model_name, suffix=args.suffix)
        else:
            # resume_path = [file for file in os.listdir(args.output_dir) if 'rd%d' % (
            #     r-1) in file]
            # print(resume_path)
            # resume_path = resume_path.sort(key=lambda x: int(x.split('_')[-2].replace('ep', '')))[0]
            # resume_path = os.path.join(args.output_dir, resume_path)
            # print('* CHOOSEN MMG GEN PTH %s' % resume_path)
            # resume_path = ""
            models_list = [int(i.replace('checkpoint_min_dice_ep', '').replace('_rd%d.pth' % (r-1), ''))
                           for i in os.listdir(args.output_dir) if i.endswith('rd%d.pth' % (r-1))]
            max_ep = max(models_list)
            model_path = os.path.join(args.output_dir, 'checkpoint_min_dice_ep%d_rd%d.pth' % (max_ep, r-1))
            model = torch.load(model_path)
            gen_mmg(modality=args.modality, root_path=args.data_path,
                    resume_path='', round_num=r, model_name=args.model_name, model=model, suffix=args.suffix)
            # del model_p  # 删除模型
            # gc.collect()  # 回收内存
            print('* USING MODEL FROM', model_path, 'TO GEN MMG')
        # sys.exit(1)
        ep_offset = one_round(r, ep_offset, model, metrics)
        ...
        print(args)
        # writer.add_scalar('best/restart', r, ep_offset)
...
if __name__ == '__main__':
    ...
    if not args.output_dir:
        args.output_dir = args.outprefx+'%s/%s_%s_%s_%s_bs%d_fn%d_kn%d' % (args.task, time.strftime(
            "%Y%m%d%H%M%S", time.localtime()), args.model_name, args.encoder, args.criterion_name, args.batch_size, args.in_channels, args.kernel_size)
